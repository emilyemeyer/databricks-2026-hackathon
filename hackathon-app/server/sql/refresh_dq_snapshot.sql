INSERT INTO dais_2026.hackathon.facility_specialty (facility_id, specialty)
SELECT DISTINCT
  f.facility_id,
  TRIM(
    regexp_replace(
...
LEFT JOIN (SELECT DISTINCT facility_id FROM dais_2026.hackathon.facility_specialty) existing
...
CREATE OR REPLACE TABLE dais_2026.hackathon.dq_metrics AS
...
district_gap AS (
  ...
  FROM (SELECT DISTINCT district_id FROM dais_2026.hackathon.health_indicator) d
  LEFT JOIN (
...
mapping_stats AS (
  SELECT
    (SELECT COUNT(*) FROM dais_2026.hackathon.facility_specialty) AS facility_specialty_rows,
    (SELECT COUNT(*) FROM dais_2026.hackathon.health_indicator_specialty) AS health_indicator_specialty_rows,
    (SELECT COUNT(*) FROM dais_2026.hackathon.specialty_category_mapping) AS specialty_category_rows,
    ...
    (SELECT COUNT(*)
     ...
     LEFT JOIN (SELECT DISTINCT indicator_key FROM dais_2026.hackathon.health_indicator) hi
    ...
    (SELECT COUNT(DISTINCT hi.indicator_key)
     FROM (SELECT DISTINCT indicator_key FROM dais_2026.hackathon.health_indicator) hi
     LEFT JOIN dais_2026.hackathon.health_indicator_specialty his ON hi.indicator_key = his.indicator_key
     WHERE his.indicator_key IS NULL) AS unmapped_indicators,
    (SELECT COUNT(*)
     FROM dais_2026.hackathon.facility f
     LEFT JOIN (SELECT DISTINCT facility_id FROM dais_2026.hackathon.facility_specialty) fs
...
CREATE OR REPLACE TABLE dais_2026.hackathon.dq_gap_staging AS
SELECT
...
FROM (
  ...
  FROM (
    SELECT DISTINCT specialty FROM dais_2026.hackathon.facility_specialty
  ) fs
  LEFT JOIN dais_2026.hackathon.specialty_category_mapping m ON fs.specialty = m.specialties
...
MERGE INTO dais_2026.hackathon.dq_gap AS target
USING dais_2026.hackathon.dq_gap_staging AS source
...
UPDATE dais_2026.hackathon.dq_gap AS target
SET
...
WHERE target.status IN ('open', 'resolved')
  AND NOT EXISTS (
    SELECT 1 FROM dais_2026.hackathon.dq_gap_staging source WHERE source.gap_id = target.gap_id
  );

DROP TABLE IF EXISTS dais_2026.hackathon.dq_gap_staging;
