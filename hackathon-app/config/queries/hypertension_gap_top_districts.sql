-- Top 15 districts by gap_score for bar chart (x = district label, y = gap_score).
WITH state_map AS (
  SELECT * FROM VALUES
    ('DELHI', 'NCT OF DELHI'),
...
scored AS (
  SELECT
    CONCAT(district_name, ', ', state_ut) AS district_label,
    hypertension_pct,
    cardiac_facilities,
    (hypertension_pct / 100.0) - COALESCE(cardiac_facilities / NULLIF(MAX(cardiac_facilities) OVER (), 0), 0) AS gap_score
  FROM joined
)
SELECT district_label, gap_score, hypertension_pct, cardiac_facilities
FROM scored
