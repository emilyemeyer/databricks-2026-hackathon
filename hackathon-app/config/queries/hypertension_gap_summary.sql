  ...
  SELECT
    ...
    CASE
      WHEN cardiac_facilities = 0
        AND hypertension_pct > (SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY hypertension_pct) FROM scored)
        THEN 'no_supply'
...
top10 AS (
  SELECT district_name, state_ut, hypertension_pct, cardiac_facilities, gap_score, gap_flag
  FROM flagged
...
SELECT
  ...
  (SELECT COUNT(*) FROM scored WHERE cardiac_facilities = 0) AS districts_with_zero_cardiac_supply,
  (SELECT ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY hypertension_pct), 2) FROM scored) AS median_hypertension_pct,
  (SELECT COUNT(*) FROM flagged WHERE gap_flag IN ('high_gap', 'no_supply')) AS districts_high_gap_or_no_supply,
  (SELECT TO_JSON(COLLECT_LIST(STRUCT(district_name, state_ut, hypertension_pct, cardiac_facilities, gap_score, gap_flag))) FROM top10) AS top_10_high_gap_districts
